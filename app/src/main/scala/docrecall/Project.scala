//> using scala 3.6.3
//> using resourceDir ../../resources

//> using options -experimental -language:experimental.betterFors

//> using toolkit typelevel:0.1.29

//> using dep org.typelevel::log4cats-slf4j:2.7.0

//> using dep com.outr::scribe-slf4j2:3.16.0
//> using dep com.outr::scribe-file:3.16.0

//> using dep org.http4s::http4s-dsl::0.23.30
//> using dep org.http4s::http4s-ember-server::0.23.30
//> using dep org.http4s::http4s-scalatags::0.25.2

//> using dep org.tpolecat::skunk-core::1.0.0-M10
//> using dep dev.rolang::dumbo::0.5.5

//> using dep com.lihaoyi::scalatags::0.13.1

//> using dep com.softwaremill.sttp.openai::fs2:0.3.0
//> using dep com.softwaremill.sttp.openai::core:0.3.0

//> using dep com.softwaremill.sttp.client4::cats:4.0.0-RC1
//> using dep com.softwaremill.sttp.client4::jsoniter:4.0.0-RC1

//> using dep io.scalaland::chimney::1.7.3

//> using dep dev.langchain4j:langchain4j:1.0.0-beta1
//> using dep dev.langchain4j:langchain4j-document-parser-apache-tika:1.0.0-beta1

//> using dep com.davegurnell::unindent:1.8.0

//> using dep com.github.plokhotnyuk.jsoniter-scala::jsoniter-scala-core::2.33.2
//> using dep com.github.plokhotnyuk.jsoniter-scala::jsoniter-scala-macros::2.33.2

//> using dep com.slack.api:slack-app-backend:1.45.3

//> using test.dep com.dimafeng::testcontainers-scala-munit::0.41.4
